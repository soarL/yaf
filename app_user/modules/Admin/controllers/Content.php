...
class ContentController extends Controller {
    ...
    public function linksAction()
    {
        ...
        $builder = Link::whereRaw('1=1');

        $links = $builder->orderBy('id','desc')->paginate(15);

        $links->appends($queries->all());

        $this->display('links', ['links'=>$links, 'queries'=>$queries]);
    ...
    {
        $link = new Link();
        $this->display('setLinks',['link'=>$link]);
    ...
    public function editLinkAction()
    {
        ...
        $this->display('setLinks',['link'=>$link, 'queries'=>$queries]);
    ...
    public function saveLinkAction()
    {
        ...
        if($params['name']=='') {
            Flash::error('链接名称不能为空！');
            $this->goBack();
        }

        $link->link_name = $params['name'];
        $link->link_url = $params['url'];
        $link->link_time = date('Y-m-d H:i:s',time());


        if($link->save()) {
            Flash::success('操作成功！');
            $this->redirect('/admin/content/links');
        } else {
            Flash::error('操作失败！');
            $this->redirect('/admin/content/setLink');
        }
    ...
    {
        $id = $this->getQuery('id');
        $re =DB::table('system_link')->where('id',$id)->delete();
        if ($re){
            $this->redirect('/admin/content/links');
        }else {
            $this->redirect('/admin/content/links');
