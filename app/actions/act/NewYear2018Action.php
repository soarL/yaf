 ...
 * @version 1.0
 */
class NewYear2018Action extends Action {
    use ModeHandle, DisplayHandle;

    public $actBegin = '2018-02-11 00:00:00';
    public $actEnd = '2018-02-29 00:00:00';
    public $packetes = [
        1=>1.6, 
        2=>1.8, 
        3=>2,
    ...
    public function shareMode(){
        $redis = RedisFactory::create();
        $user = $this->getUser();
        $count = $redis->get('share:'.$user->userId);
        if(!$count){
            $redis->incr('year:'.$user->userId);
            $redis->set('share:'.$user->userId,1);
        }
    }

    public function actDataMode(){
        $redis = RedisFactory::create();
        ...
        if($user){
            $count = $redis->get('year:'.$user->userId);
            $yearCount = $redis->get('yearCount:'.$user->userId);
            if($count === false && $yearCount == 0) {
                $count = 2;
                $redis->set('year:'.$user->userId,2);
                $redis->set('yearCount:'.$user->userId,0);
            }
            $sum = ActUserPacket::where('userId',$user->userId)->sum('money');
            $get = $sum ? $sum : $get;
        }

        $pack = ActUserPacket::with('user')->limit(30)->get();
        $html = '';
        foreach ($pack as $key => $value) {
            if($value->user){
                $name = _hide_name($value->user->name);
                $phone = _hide_phone($value->user->phone);
            }else{
                $name = _hide_name($value->name);
                $phone = _hide_phone($value->phone);
            }
            $money = $value->money;
            if($value->status){
                $color = '1d5bc4';
            ...
            $html .= '<tr><td>'.$name.'</td><td>'.$phone.'</td><td>'.$money.'元</td><td style="color:#'.$color.'">'.$msg.'</td></tr>';
        }
        
        $total = number_format($redis->get('prize')/100,2);
        if($total < 1.6){
            $total = 0.00;
        }
        ...
        $params['url'] = urldecode($this->getQuery('url'));
        $paramsSort = StringHelper::paramsSort($params);
    ...
    public function getaccessToken(){
        ...
        if($access_token){
            ...
            $res = NetworkHelper::CurlPost('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wx2973416012c3598e&secret=d732b488fd9d8c37513e87ec26e0205b','');
            $data = json_decode($res,true);
            $access_token = $data['access_token'];
            $redis->setex('access_token','3600',$access_token);
            return $access_token;
        }
    }
    ...
    public function getTicket(){
        ...
        if($ticket){
            ...
            $res = NetworkHelper::CurlPost('https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token='.$access_token.'&type=jsapi','');
            $data = json_decode($res,true);
            $ticket = $data['ticket'];
            $redis->setex('ticket','3600',$ticket);
            return $ticket;
        }
    }
    ...
    public function lotteryDrawMode() {
        ...
        if($prizeID) {
            ...
            $packet->money = $this->packetes[$prizeID];
            $packet->save();

            $redis->incr('yearCount:'.$user->userId);
            $redis->decr('year:'.$user->userId);

            if($yearCount == 1){
                $list = [];
                $params = [];
                $params['type'] = 'money';
                $params['useful_day'] = '30';
                $params['remark'] = '[活动]新年送红包';
                $params['userId'] = $user->userId;
                $params['money_rate'] = '5';
                $params['money_lower'] = '200';
                $list[] = $params;
    ...
    public function getPrize() {
        ...
        $id = rand(1, 12);
        $prizeId = false;
        $money = $packetes[$id]*100;
        if($this->hasPrize($money)){
            $prizeId = $id;
        }
        return $prizeId;
    }
     ...
     * @return boolean
     */
    public function hasPrize($money) {
        $redis = RedisFactory::create();
        $prize = $redis->get('prize');
        if($prize > $money) {
            $redis->set('prize',$prize - $money);
            return true;
        } else {
            return false;
        }
