...
class TCPDFFonts {
	...
	public static function UTF8ArrayToUniArray($ta, $isunicode=true) {
		if ($isunicode) {
			return array_map(array('plugins\tcpdf\tools\TCPDFFonts', 'unichrUnicode'), $ta);
		}
		return array_map(array('plugins\tcpdf\tools\TCPDFFonts', 'unichrASCII'), $ta);
	}
	...
	public static function UTF8StringToArray($str, $isunicode=true, &$currentfont) {
		if ($isunicode) {
			...
			$carr = array_map(array('plugins\tcpdf\tools\TCPDFFonts', 'uniord'), $chars);
		} else {
			$chars = str_split($str);
