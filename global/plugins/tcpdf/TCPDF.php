...
use plugins\tcpdf\tools\TCPDFColors;
use plugins\tcpdf\tools\TCPDFFontData;
use plugins\tcpdf\tools\TCPDFFonts;
use plugins\tcpdf\tools\TCPDFImages;
use plugins\tcpdf\tools\TCPDFStatic;
use \Exception;
class TCPDF {
	protected $K_PATH_CACHE = '/tmp/';
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (TCPDFStatic::empty_string($fontfile) OR (!@file_exists($fontfile))) {
			...
			$fontfile = TCPDFFonts::getFontFullPath($tmp_fontfile, $fontdir);
			if (TCPDFStatic::empty_string($fontfile)) {
				$missing_style = true;
		...
		// include font file
		if (!TCPDFStatic::empty_string($fontfile) AND (@file_exists($fontfile))) {
			include($fontfile);
		} else {
	 ...
	 */
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=100, $valign='M', $fitcell=false) {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (in_array($file, $this->imagekeys)) {
			...
			$tempfile_plain = $this->K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_imgmask_plain_'.$filehash;
			// create temp alpha file
			$tempfile_alpha = $this->K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_imgmask_alpha_'.$filehash;
		...
		if ($newimage) {
			...
			$info = false;
			if ((method_exists('plugins\tcpdf\tools\TCPDFImages', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
			...
			if (($info === false) AND function_exists($gdfunction)) {
				try {
					...
					if ($img !== false) {
						...
						if (($type == 'gif') OR ($type == 'png')) {
							$info = TCPDFImages::_toPNG($img, TCPDFStatic::getObjFilename('img', $this->file_id));
						} else {
							$info = TCPDFImages::_toJPEG($img, $this->jpeg_quality, TCPDFStatic::getObjFilename('img', $this->file_id));
						}
					}
				} catch(Exception $e) {
					$info = false;
	...
	public function _destroy($destroyall=false, $preserve_objcopy=false) {
		if ($destroyall AND !$preserve_objcopy && property_exists($this, 'K_PATH_CACHE') && @$this->K_PATH_CACHE!=null) {
			// remove all temporary files
			$tmpfiles = glob($this->K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_*');
			if (!empty($tmpfiles)) {
	...
	protected function _puttruetypeunicode($font) {
		...
		if (isset($font['ctg']) AND (!TCPDFStatic::empty_string($font['ctg']))) {
			...
			// search and get ctg font file to embedd
			$fontfile = TCPDFFonts::getFontFullPath($ctgfile, $fontdir);
			if (TCPDFStatic::empty_string($fontfile)) {
	 ...
	 * @param $pubkeys (String) array of recipients containing public-key oddPropertyPhotos ('c') and permissions ('p'). For example: array(array('c' => 'file://../examples/data/cert/tcpdf.crt', 'p' => array('print')))
	 * @public
	 * @since 2.0.000 (2008-01-02)
	 * @author Nicola Asuni
	 */
	 ...
	 * @param $extracerts (string) specifies the name of a file containing a bunch of extra oddPropertyPhotos to include in the signature which can for example be used to help the recipient to verify the certificate that you used.
		...
		foreach ($bars as $col) {
			switch ($col) {
				...
				default: { // SPECIFIC SPOT COLOR NAME
					$col_a = array(0,0,0,0,'None');
					$col_b = TCPDFColors::getSpotColor($col, $this->spot_colors);
					if ($col_b === false) {
