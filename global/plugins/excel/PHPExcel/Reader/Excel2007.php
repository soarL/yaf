 ...
 */
namespace plugins\excel\PHPExcel\Reader;

use plugins\excel\PHPExcel\Shared\SharedString;
use plugins\excel\PHPExcel\Reader\Exception as ReaderException;
use plugins\excel\PHPExcel\DocumentProperties;
use plugins\excel\PHPExcel\Settings;
use plugins\excel\PHPExcel\Shared\Font as SharedFont;
use plugins\excel\PHPExcel\Shared\File as SharedFile;
use plugins\excel\PHPExcel\Shared\Date as SharedDate;
use plugins\excel\PHPExcel\Worksheet\AutoFilter\Column;
use plugins\excel\PHPExcel\Worksheet\AutoFilter\Column\Rule;
use plugins\excel\PHPExcel\Style\Borders;
use plugins\excel\PHPExcel\Style\Protection;
use plugins\excel\PHPExcel\Style\Color;
use plugins\excel\PHPExcel\Style\Conditional;
use plugins\excel\PHPExcel\Style\NumberFormat;
use plugins\excel\PHPExcel\Shared\Drawing as SharedDrawing;
use plugins\excel\PHPExcel\Reader\Excel2007\Theme;
use plugins\excel\PHPExcel\Reader\Excel2007\Chart;
use plugins\excel\PHPExcel\Worksheet\HeaderFooterDrawing;
use plugins\excel\PHPExcel\Style\Font as StyleFont;;
use plugins\excel\PHPExcel\Worksheet\Drawing as WorksheetDrawing;

use plugins\excel\PHPExcel\Style;
use plugins\excel\PHPExcel\Cell;
use plugins\excel\PHPExcel\RichText;
use plugins\excel\PHPExcel\NamedRange;
use plugins\excel\PHPExcel\Worksheet;
use plugins\excel\PHPExcel\ReferenceHelper;
use plugins\excel\PHPExcel;
 ...
 * @copyright	Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class Excel2007 extends ReaderAbstract implements IReader
{
	/**
	...
	public function __construct() {
		$this->_readFilter = new DefaultReadFilter();
...
//			throw new ReaderException($zipClass . " library is not enabled");
//		}
		...
		if ($zip->open($pFilename) === true) {
			...
			$rels = simplexml_load_string($this->_getFromZipArchive($zip, "_rels/.rels"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
			if ($rels !== false) {
	...
	public function listWorksheetNames($pFilename)
	{
		...
		$rels = simplexml_load_string(
		    $this->_getFromZipArchive($zip, "_rels/.rels", 'SimpleXMLElement', Settings::getLibXmlLoaderOptions())
		); //~ http://schemas.openxmlformats.org/package/2006/relationships");
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					$xmlWorkbook = simplexml_load_string(
					    $this->_getFromZipArchive($zip, "{$rel['Target']}", 'SimpleXMLElement', Settings::getLibXmlLoaderOptions())
	...
	public function listWorksheetInfo($pFilename)
	{
		...
		$rels = simplexml_load_string($this->_getFromZipArchive($zip, "_rels/.rels"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
		foreach ($rels->Relationship as $rel) {
			if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
				$dir = dirname($rel["Target"]);
				$relsWorkbook = simplexml_load_string($this->_getFromZipArchive($zip, "$dir/_rels/" . basename($rel["Target"]) . ".rels"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/package/2006/relationships");
				...
				$xmlWorkbook = simplexml_load_string($this->_getFromZipArchive($zip, "{$rel['Target']}"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
				if ($xmlWorkbook->sheets) {
					$dir = dirname($rel["Target"]);
					foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
						$tmpInfo = array(
						...
						$xml = new XMLReader();
						$res = $xml->open('zip://'.SharedFile::realpath($pFilename).'#'."$dir/$fileWorksheet", null, Settings::getLibXmlLoaderOptions());
...
//				echo 'Adjusted Formula is ', $value, PHP_EOL;
	...
	public function load($pFilename)
	{
		...
		$wbRels = simplexml_load_string($this->_getFromZipArchive($zip, "xl/_rels/workbook.xml.rels"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
		foreach ($wbRels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
					...
					$xmlTheme = simplexml_load_string($this->_getFromZipArchive($zip, "xl/{$rel['Target']}"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
					if (is_object($xmlTheme)) {
						...
						self::$_theme = new Theme($themeName,$colourSchemeName,$themeColours);
					}
					break;
			}
		}
		...
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties":
					$xmlCore = simplexml_load_string($this->_getFromZipArchive($zip, "{$rel['Target']}"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					$xmlStrings = simplexml_load_string($this->_getFromZipArchive($zip, "$dir/$xpath[Target]"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
					...
					$xmlStyles = simplexml_load_string($this->_getFromZipArchive($zip, "$dir/$xpath[Target]"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
					$numFmts = null;
					...
					if ($xmlWorkbook->sheets) {
						foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
							...
							$fileWorksheet = $worksheets[(string) self::array_item($eleSheet->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "id")];
							$xmlSheet = simplexml_load_string($this->_getFromZipArchive($zip, "$dir/$fileWorksheet"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
	...
	//									echo 'Value is ', $value, PHP_EOL;
							...
							if (!$this->_readDataOnly) {
								...
								if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
									$relsWorksheet = simplexml_load_string($this->_getFromZipArchive($zip,  dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels") , 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
							...
							if (!$this->_readDataOnly) {
								...
								foreach ($comments as $relName => $relPath) {
									...
									$commentsFile = simplexml_load_string($this->_getFromZipArchive($zip, $relPath) , 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
							    ...
							    foreach ($vmlComments as $relName => $relPath) {
									...
									$relPath = SharedFile::realpath(dirname("$dir/$fileWorksheet") . "/" . $relPath);
									$vmlCommentsFile = simplexml_load_string( $this->_getFromZipArchive($zip, $relPath) , 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
								...
								if ($xmlSheet && $xmlSheet->legacyDrawingHF && !$this->_readDataOnly) {
									if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
										...
										if ($vmlRelationship != '') {
											// Fetch linked images
											$relsVML = simplexml_load_string($this->_getFromZipArchive($zip,  dirname($vmlRelationship) . '/_rels/' . basename($vmlRelationship) . '.rels' ), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
											$drawings = array();
											...
											// Fetch VML document
											$vmlDrawing = simplexml_load_string($this->_getFromZipArchive($zip, $vmlRelationship), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
							...
							if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
								...
								if ($xmlSheet->drawing && !$this->_readDataOnly) {
									foreach ($xmlSheet->drawing as $drawing) {
										...
										$relsDrawing = simplexml_load_string($this->_getFromZipArchive($zip,  dirname($fileDrawing) . "/_rels/" . basename($fileDrawing) . ".rels") , 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
										$images = array();
										...
										$xmlDrawing = simplexml_load_string($this->_getFromZipArchive($zip, $fileDrawing), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions())->children("http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing");
										...
										if ($xmlDrawing->oneCellAnchor) {
											foreach ($xmlDrawing->oneCellAnchor as $oneCellAnchor) {
												if ($oneCellAnchor->pic->blipFill) {
													...
													$objDrawing->setPath("zip://".SharedFile::realpath($pFilename)."#" . $images[(string) self::array_item($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], false);
													$objDrawing->setCoordinates(Cell::stringFromColumnIndex((string) $oneCellAnchor->from->col) . ($oneCellAnchor->from->row + 1));
													$objDrawing->setOffsetX(SharedDrawing::EMUToPixels($oneCellAnchor->from->colOff));
													$objDrawing->setOffsetY(SharedDrawing::EMUToPixels($oneCellAnchor->from->rowOff));
													$objDrawing->setResizeProportional(false);
													$objDrawing->setWidth(SharedDrawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cx")));
													$objDrawing->setHeight(SharedDrawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cy")));
													if ($xfrm) {
														$objDrawing->setRotation(SharedDrawing::angleToDegrees(self::array_item($xfrm->attributes(), "rot")));
													}
													if ($outerShdw) {
														$shadow = $objDrawing->getShadow();
														$shadow->setVisible(true);
														$shadow->setBlurRadius(SharedDrawing::EMUTopixels(self::array_item($outerShdw->attributes(), "blurRad")));
														$shadow->setDistance(SharedDrawing::EMUTopixels(self::array_item($outerShdw->attributes(), "dist")));
														$shadow->setDirection(SharedDrawing::angleToDegrees(self::array_item($outerShdw->attributes(), "dir")));
													...
													$offsetX		= SharedDrawing::EMUToPixels($oneCellAnchor->from->colOff);
													$offsetY		= SharedDrawing::EMUToPixels($oneCellAnchor->from->rowOff);
													$width			= SharedDrawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cx"));
													$height			= SharedDrawing::EMUToPixels(self::array_item($oneCellAnchor->ext->attributes(), "cy"));
												}
											}
										}
										if ($xmlDrawing->twoCellAnchor) {
											foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
												if ($twoCellAnchor->pic->blipFill) {
													...
													$objDrawing->setOffsetX(SharedDrawing::EMUToPixels($twoCellAnchor->from->colOff));
													$objDrawing->setOffsetY(SharedDrawing::EMUToPixels($twoCellAnchor->from->rowOff));
													$objDrawing->setResizeProportional(false);

													$objDrawing->setWidth(SharedDrawing::EMUToPixels(self::array_item($xfrm->ext->attributes(), "cx")));
													$objDrawing->setHeight(SharedDrawing::EMUToPixels(self::array_item($xfrm->ext->attributes(), "cy")));
													...
													$fromOffsetX	= SharedDrawing::EMUToPixels($twoCellAnchor->from->colOff);
													$fromOffsetY	= SharedDrawing::EMUToPixels($twoCellAnchor->from->rowOff);
													$toCoordinate	= Cell::stringFromColumnIndex((string) $twoCellAnchor->to->col) . ($twoCellAnchor->to->row + 1);
													$toOffsetX		= SharedDrawing::EMUToPixels($twoCellAnchor->to->colOff);
													$toOffsetY		= SharedDrawing::EMUToPixels($twoCellAnchor->to->rowOff);
		...
		if (!$this->_readDataOnly) {
			$contentTypes = simplexml_load_string($this->_getFromZipArchive($zip, "[Content_Types].xml"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
			foreach ($contentTypes->Override as $contentType) {
				switch ($contentType["ContentType"]) {
					case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
						if ($this->_includeCharts) {
							$chartEntryRef = ltrim($contentType['PartName'],'/');
							$chartElements = simplexml_load_string($this->_getFromZipArchive($zip, $chartEntryRef), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
...
//		}
		...
		if (isset($style->font)) {
			...
			if (isset($style->font->u) && !isset($style->font->u["val"])) {
				$docStyle->getFont()->setUnderline(StyleFont::UNDERLINE_SINGLE);
			...
			foreach ($is->r as $run) {
				if (!isset($run->rPr)) {
					...
					if (isset($run->rPr->color)) {
						$objText->getFont()->setColor( new Color( self::_readColor($run->rPr->color) ) );
					}
					...
					if (isset($run->rPr->u) && !isset($run->rPr->u["val"])) {
						$objText->getFont()->setUnderline(StyleFont::UNDERLINE_SINGLE);
	...
	private function _readRibbon($excel, $customUITarget, $zip)
    {
		...
		if ($dataRels) {
			...
			$UIRels = simplexml_load_string($dataRels, 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
			if ($UIRels) {
