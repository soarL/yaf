 ...
 * @copyright	Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class OOCalc extends ReaderAbstract implements IReader
{
	/**
	 * Formats
	 *
...
//		}
		...
		if ($zip->open($pFilename) === true) {
			...
			if ($stat && ($stat['size'] <= 255)) {
		        ...
		        $xml = simplexml_load_string($zip->getFromName('META-INF/manifest.xml'), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
	...
	public function listWorksheetNames($pFilename)
	{
		...
		$xml = new XMLReader();
		$res = $xml->open('zip://'.realpath($pFilename).'#content.xml', null, Settings::getLibXmlLoaderOptions());
...
//		echo '<h1>Meta Information</h1>';
		$xml = simplexml_load_string($zip->getFromName("meta.xml"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
...
//		echo '<h1>Workbook Content</h1>';
		$xml = simplexml_load_string($zip->getFromName("content.xml"), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
		$namespacesContent = $xml->getNamespaces(true);
//		echo '<pre>';
...
//									echo 'Value Type is '.$cellDataOfficeAttributes['value-type'].'<br />';
									switch ($cellDataOfficeAttributes['value-type']) {
										...
										case 'date' :
												...
												$dataValue = SharedDate::FormattedPHPToExcel($year,$month,$day,$hour,$minute,$second);
										...
										case 'time' :
												$type = DataType::TYPE_NUMERIC;
												$dataValue = SharedDate::PHPToExcel(strtotime('01-01-1970 '.implode(':',sscanf($cellDataOfficeAttributes['time-value'],'PT%dH%dM%dS'))));
...
//													echo 'Forumla result is '.$dataValue.'<br />';
												...
												if ($formatting !== NULL) {
													...
													$objPHPExcel->getActiveSheet()->getStyle($columnID.$rID)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_GENERAL);
												}
												if ($hyperlink !== NULL) {
								...
								if ((isset($cellDataTableAttributes['number-columns-spanned'])) || (isset($cellDataTableAttributes['number-rows-spanned']))) {
									if (($type !== DataType::TYPE_NULL) || (!$this->_readDataOnly)) {
										$columnTo = $columnID;
