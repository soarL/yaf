...
use plugins\excel\PHPExcel\Shared\OLERead;
use plugins\excel\PHPExcel\Shared\OLE;
use plugins\excel\PHPExcel\Shared\CodePage;
use plugins\excel\PHPExcel\NamedRange;
use plugins\excel\PHPExcel\Exception as PHPExcelException;
use plugins\excel\PHPExcel\Reader\Exception as ReaderException;
use plugins\excel\PHPExcel\Reader\Excel5\Escher;
...
use plugins\excel\PHPExcel\Shared\Escher\DggContainer\BstoreContainer\BSE;
use plugins\excel\PHPExcel\Worksheet\MemoryDrawing;
use plugins\excel\PHPExcel\Style\Border;
use plugins\excel\PHPExcel\Style\Fill;
use plugins\excel\PHPExcel\Style\Font;
...
use plugins\excel\PHPExcel\Reader\Excel5\MD5 as Excel5MD5;
use plugins\excel\PHPExcel\Reader\Excel5\RC4 as Excel5RC4;
use plugins\excel\PHPExcel\Cell\DataValidation;
use plugins\excel\PHPExcel\Style\Alignment;
...
use plugins\excel\PHPExcel\Worksheet\Drawing;
 ...
 * @copyright	Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class Excel5 extends ReaderAbstract implements IReader
{
	// ParseXL definitions
	...
	public function load($pFilename)
	{
		...
		if (!$this->_readDataOnly && $this->_drawingGroupData) {
			$escherWorkbook = new SharedEscher();
			$reader = new Escher($escherWorkbook);
			$escherWorkbook = $reader->load($this->_drawingGroupData);

			// debug Escher stream
			//$debug = new Debug_Escher(new SharedEscher());
...
//				echo '<br />';
				...
				if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
					...
					$width = SharedExcel5::getDistanceX($this->_phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
					$height = SharedExcel5::getDistanceY($this->_phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

					// calculate offsetX and offsetY of the shape
					$offsetX = $startOffsetX * SharedExcel5::sizeCol($this->_phpSheet, $startColumn) / 1024;
					$offsetY = $startOffsetY * SharedExcel5::sizeRow($this->_phpSheet, $startRow) / 256;
			...
			if ($this->_version == self::XLS_BIFF8) {
				foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
					...
					if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($column, $row, $this->_phpSheet->getTitle()) ) {
						...
						$this->_phpSheet->getCell($cell)->setValueExplicit('=' . $formula, DataType::TYPE_FORMULA);
					}
				}
			}
...
//					echo '<br />';
	...
	private function _readRecordData($data, $pos, $len)
	{
		...
		if ($this->_encryption == self::MS_BIFF_CRYPTO_RC4) {
			...
			throw new ReaderException('XOr encryption not supported');
		}
		return $recordData;
	}
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-time
					$value = OLE::OLE2LocalDate(substr($this->_summaryInformation, $secOffset + 4 + $offset, 8));
					break;
...
//			echo 'Type is ',$type,', ';
			...
			switch ($type) {
				...
				case 0x40:	//	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-Time
					$value = OLE::OLE2LocalDate(substr($this->_documentSummaryInformation, $secOffset + 4 + $offset, 8));
					break;
...
//		echo '<br />';
		...
		switch ($substreamType) {
			case self::XLS_WorkbookGlobals:
				...
				if (($version != self::XLS_BIFF8) && ($version != self::XLS_BIFF7)) {
					throw new ReaderException('Cannot read this Excel file. Version is too old.');
				}
	...
	private function _readFilepass()
	{
		...
		if ($length != 54) {
			throw new ReaderException('Unexpected file pass record length');
		}
		...
		if (!$this->_verifyPassword(
			...
			throw new ReaderException('Decryption password incorrect');
		}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (!$isNotInit) {
				...
				switch ($isPortrait) {
				case 0: $this->_phpSheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE); break;
				case 1: $this->_phpSheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_PORTRAIT); break;
				}
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = SharedString::Substring($this->_sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
						$charPos = $fmtRuns[$i]['charPos'];
					} else {
						$text = SharedString::Substring($this->_sst[$index]['value'], $charPos, SharedString::CountCharacters($this->_sst[$index]['value']));
					}
				...
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($this->_sst[$index]['value'], DataType::TYPE_STRING);
			}
	...
	private function _readFormula()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (!$isPartOfSharedFormula) {
				...
				try {
					if ($this->_version != self::XLS_BIFF8) {
						throw new ReaderException('Not BIFF8. Can only read BIFF8 formulas');
					}
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		if ($this->_phpSheet->getSheetView()->getView() !== SheetView::SHEETVIEW_PAGE_LAYOUT) {
			...
			$view = $isPageBreakPreview? SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW :
	...
	private function _readPageLayoutView(){
		...
		if ($fPageLayoutView === 1) {
			$this->_phpSheet->getSheetView()->setView(SheetView::SHEETVIEW_PAGE_LAYOUT);
	...
	private function _readDataValidation()
	{
		...
		switch ($type) {
			case 0x00:	$type = DataValidation::TYPE_NONE;		break;
			case 0x01:	$type = DataValidation::TYPE_WHOLE;		break;
			case 0x02:	$type = DataValidation::TYPE_DECIMAL;		break;
			case 0x03:	$type = DataValidation::TYPE_LIST;		break;
			case 0x04:	$type = DataValidation::TYPE_DATE;		break;
			case 0x05:	$type = DataValidation::TYPE_TIME;		break;
			case 0x06:	$type = DataValidation::TYPE_TEXTLENGTH;	break;
			case 0x07:	$type = DataValidation::TYPE_CUSTOM;		break;
		}

		// bit: 4-6; mask: 0x00000070; error type
		$errorStyle = (0x00000070 & $options) >> 4;
		switch ($errorStyle) {
			case 0x00:	$errorStyle = DataValidation::STYLE_STOP;			break;
			case 0x01:	$errorStyle = DataValidation::STYLE_WARNING;		break;
			case 0x02:	$errorStyle = DataValidation::STYLE_INFORMATION;	break;
		}
		...
		switch ($operator) {
			case 0x00: $operator = DataValidation::OPERATOR_BETWEEN			;	break;
			case 0x01: $operator = DataValidation::OPERATOR_NOTBETWEEN		;	break;
			case 0x02: $operator = DataValidation::OPERATOR_EQUAL				;	break;
			case 0x03: $operator = DataValidation::OPERATOR_NOTEQUAL			;	break;
			case 0x04: $operator = DataValidation::OPERATOR_GREATERTHAN		;	break;
			case 0x05: $operator = DataValidation::OPERATOR_LESSTHAN			;	break;
			case 0x06: $operator = DataValidation::OPERATOR_GREATERTHANOREQUAL;	break;
			case 0x07: $operator = DataValidation::OPERATOR_LESSTHANOREQUAL	;	break;
		}
...
//			var_dump($bcBitCount);
	...
	private function _getNextToken($formulaData, $baseCell = 'A1')
	{
		...
		case 0x19:	//	Special attribute
			...
			case 0x41:
				...
				case 0x05:
					...
					break;
				default:
					throw new ReaderException('Unrecognized space type in tAttrSpace token');
					break;
				}
				...
				break;
			default:
				throw new ReaderException('Unrecognized attribute flag in tAttr token');
				break;
			}
			break;
		...
		case 0x61:
			...
			default:
				throw new ReaderException('Unrecognized function in formula');
				break;
			}
		...
		default:
			throw new ReaderException('Unrecognized token ' . sprintf('%02X', $id) . ' in formula');
			break;
		}
	 ...
	 * in which case an ReaderException is thrown
	 *
	 * @param int $index
	 * @return string|false
	 * @throws ReaderException
			...
			switch ($type) {
				case 'internal':
					...
					if ($this->_ref[$index]['firstSheetIndex'] == 0xFFFF or $this->_ref[$index]['lastSheetIndex'] == 0xFFFF) {
						throw new ReaderException('Deleted sheet reference');
					}
				...
				default:
					...
					throw new ReaderException('Excel5 reader only supports internal sheets in fomulas');
					break;
			}
		}
		return false;
	...
	{
		return SharedString::ConvertEncoding($string, 'UTF-8', $this->_codepage);
	}
	...
	{
		switch ($index) {
			case 0x00: return Border::BORDER_NONE;
			case 0x01: return Border::BORDER_THIN;
			case 0x02: return Border::BORDER_MEDIUM;
			case 0x03: return Border::BORDER_DASHED;
			case 0x04: return Border::BORDER_DOTTED;
			case 0x05: return Border::BORDER_THICK;
			case 0x06: return Border::BORDER_DOUBLE;
			case 0x07: return Border::BORDER_HAIR;
			case 0x08: return Border::BORDER_MEDIUMDASHED;
			case 0x09: return Border::BORDER_DASHDOT;
			case 0x0A: return Border::BORDER_MEDIUMDASHDOT;
			case 0x0B: return Border::BORDER_DASHDOTDOT;
			case 0x0C: return Border::BORDER_MEDIUMDASHDOTDOT;
			case 0x0D: return Border::BORDER_SLANTDASHDOT;
	...
	{
		switch ($index) {
			case 0x00: return Fill::FILL_NONE;
			case 0x01: return Fill::FILL_SOLID;
			case 0x02: return Fill::FILL_PATTERN_MEDIUMGRAY;
			case 0x03: return Fill::FILL_PATTERN_DARKGRAY;
			case 0x04: return Fill::FILL_PATTERN_LIGHTGRAY;
			case 0x05: return Fill::FILL_PATTERN_DARKHORIZONTAL;
			case 0x06: return Fill::FILL_PATTERN_DARKVERTICAL;
			case 0x07: return Fill::FILL_PATTERN_DARKDOWN;
			case 0x08: return Fill::FILL_PATTERN_DARKUP;
			case 0x09: return Fill::FILL_PATTERN_DARKGRID;
			case 0x0A: return Fill::FILL_PATTERN_DARKTRELLIS;
			case 0x0B: return Fill::FILL_PATTERN_LIGHTHORIZONTAL;
			case 0x0C: return Fill::FILL_PATTERN_LIGHTVERTICAL;
			case 0x0D: return Fill::FILL_PATTERN_LIGHTDOWN;
			case 0x0E: return Fill::FILL_PATTERN_LIGHTUP;
			case 0x0F: return Fill::FILL_PATTERN_LIGHTGRID;
			case 0x10: return Fill::FILL_PATTERN_LIGHTTRELLIS;
			case 0x11: return Fill::FILL_PATTERN_GRAY125;
			case 0x12: return Fill::FILL_PATTERN_GRAY0625;
			default:   return Fill::FILL_NONE;
		}
	}
