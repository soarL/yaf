...
use plugins\excel\PHPExcel\Worksheet\PageMargins;
use plugins\excel\PHPExcel\Worksheet\HeaderFooter;
use plugins\excel\PHPExcel\Worksheet\SheetView;
use plugins\excel\PHPExcel\Worksheet\Protection;
use plugins\excel\PHPExcel\Worksheet\RowDimension;
use plugins\excel\PHPExcel\Worksheet\ColumnDimension;
...
use plugins\excel\PHPExcel\Shared\Font;
use plugins\excel\PHPExcel\Style\Conditional;
use plugins\excel\PHPExcel\Shared\PasswordHasher;
use plugins\excel\PHPExcel\Worksheet\RowIterator;
use plugins\excel\PHPExcel\Style\Color;
use plugins\excel\PHPExcel\Cell\Hyperlink;
...
class Worksheet implements IComparable
{
    ...
    */
    ...
    public function __construct(PHPExcel $pParent = null, $pTitle = 'Worksheet')
    {
        ...
        $this->_headerFooter        = new HeaderFooter();

        // Set sheet view
        $this->_sheetView            = new SheetView();
        ...
        // Default row dimension
        $this->_defaultRowDimension = new RowDimension(NULL);

        // Default column dimension
        $this->_defaultColumnDimension    = new ColumnDimension(NULL);

        $this->_autoFilter            = new AutoFilter(NULL, $this);
    }
   ...
   /**
    ...
    public function getRowDimension($pRow = 1, $create = TRUE)
    {
        ...
        if (!isset($this->_rowDimensions[$pRow])) {
			if (!$create)
				return NULL;
            $this->_rowDimensions[$pRow] = new RowDimension($pRow);
    ...
    public function getColumnDimension($pColumn = 'A', $create = TRUE)
    {
        ...
        if (!isset($this->_columnDimensions[$pColumn])) {
			if (!$create)
				return NULL;
            $this->_columnDimensions[$pColumn] = new ColumnDimension($pColumn);
    ...
    */
	...
	public function getRowIterator($startRow = 1) {
        return new RowIterator($this,$startRow);
    }
...
//
    ...
    public function getHyperlink($pCellCoordinate = 'A1')
    {
        ...
        $this->_hyperlinkCollection[$pCellCoordinate] = new Hyperlink();
        return $this->_hyperlinkCollection[$pCellCoordinate];
    }
    ...
    public function getDataValidation($pCellCoordinate = 'A1')
    {
        ...
        $this->_dataValidationCollection[$pCellCoordinate] = new DataValidation();
        return $this->_dataValidationCollection[$pCellCoordinate];
    }
	...
	public function __clone() {
        foreach ($this as $key => $val) {
            ...
            if (is_object($val) || (is_array($val))) {
                ...
                } elseif (($key == '_autoFilter') && ($this->_autoFilter instanceof AutoFilter)) {
