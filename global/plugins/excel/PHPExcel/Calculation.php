...
use plugins\excel\PHPExcel\Calculation\Exception as CalculationException;
use plugins\excel\PHPExcel\Calculation\CalFunction;
use plugins\excel\PHPExcel\Calculation\Functions;
use plugins\excel\PHPExcel\CalcEngine\CyclicReferenceStack;
use plugins\excel\PHPExcel\CalcEngine\Logger;
use plugins\excel\PHPExcel\Calculation\Token\Stack;
use plugins\excel\PHPExcel\Shared\SharedString;
use plugins\excel\PHPExcel\Shared\JAMA\Matrix;
...
class Calculation {
	...
	private function __construct(PHPExcel $workbook = NULL) {
		...
		$this->_cyclicReferenceStack = new CyclicReferenceStack();
	    $this->_debugLog = new Logger($this->_cyclicReferenceStack);
	...
	public final function __clone() {
		throw new CalculationException ('Cloning the calculation engine is not allowed!');
	}	//	function __clone()
			...
			if ($locale != 'en_us') {
				...
				foreach ($localeFunctions as $localeFunction) {
					...
					if (strpos($localeFunction,'=') !== FALSE) {
						...
						$fName = trim($fName);
						$lfName = trim($lfName);
						if ((isset(PHPExcelFunctions::$list[$fName])) && ($lfName != '') && ($fName != $lfName)) {
	...
	public function calculateCellValue(Cell $pCell = NULL, $resetLog = TRUE) {
		...
		if ((is_array($result)) && (self::$returnArrayAsType != self::RETURN_ARRAY_AS_ARRAY)) {
			...
			if (count($testResult) != 1) {
				...
				$r = array_keys($result);
				$r = array_shift($r);
				if (!is_numeric($r)) { return Functions::VALUE(); }
...
//echo 'Built-in function '.$functionName,PHP_EOL;
					...
					} elseif (isset(PHPExcelFunctions::$list[$functionName])) {
//echo 'PHPExcel function '.$functionName,PHP_EOL;
						$expectedArgumentCount = PHPExcelFunctions::$list[$functionName]['argumentCount'];
						$functionCall = PHPExcelFunctions::$list[$functionName]['functionCall'];
...
//					echo 'Element '.$val.' is a Function<br />';
					if (isset(PHPExcelFunctions::$list[strtoupper($matches[1])]) || isset(self::$_controlFunctions[strtoupper($matches[1])])) {	// it's a function
...
//				echo 'Token is a function<br />';
				...
				if ((isset(PHPExcelFunctions::$list[$functionName])) || (isset(self::$_controlFunctions[$functionName]))) {	// function
					if (isset(PHPExcelFunctions::$list[$functionName])) {
						$functionCall = PHPExcelFunctions::$list[$functionName]['functionCall'];
						$passByReference = isset(PHPExcelFunctions::$list[$functionName]['passByReference']);
						$passCellReference = isset(PHPExcelFunctions::$list[$functionName]['passCellReference']);
...
//					echo 'Function '.$functionName.' expects '.$argCount.' arguments<br />';
					...
					for ($i = 0; $i < $argCount; ++$i) {
						...
						$a = $argCount - $i - 1;
						if (($passByReference) &&
							(isset(PHPExcelFunctions::$list[$functionName]['passByReference'][$a])) &&
							(PHPExcelFunctions::$list[$functionName]['passByReference'][$a])) {
...
//					echo '<br />';
					if ($functionName != 'MKMATRIX') {
						if ($this->_debugLog->getWriteDebugLog()) {
							krsort($argArrayVals);
							$this->_debugLog->writeDebugLog('Evaluating ', self::_localeFunc($functionName), '( ', implode(self::$_localeArgumentSeparator.' ',Functions::flattenArray($argArrayVals)), ' )');
						}
					}
...
//		}
	...
	protected function _raiseFormulaError($errorMessage) {
		...
		$this->_cyclicReferenceStack->clear();
		if (!$this->suppressFormulaErrors) throw new CalculationException($errorMessage);
...
//			echo '<br />';
	...
	public function isImplemented($pFunction = '') {
		var_dump('isImplemented');
		$pFunction = strtoupper ($pFunction);
		if (isset(PHPExcelFunctions::$list[$pFunction])) {
			return (PHPExcelFunctions::$list[$pFunction]['functionCall'] != PHPEXCEL_SPACE.'PHPExcel\Calculation\Functions::DUMMY');
		} else {
			return FALSE;
		}
	...
	public function listFunctions() {
		var_dump('listFunctions');
		// Return value
		$returnValue = array();
		// Loop functions
		foreach(PHPExcelFunctions::$list as $functionName => $function) {
			if ($function['functionCall'] != PHPEXCEL_SPACE.'PHPExcel\Calculation\Functions::DUMMY') {
				$returnValue[$functionName] = new CalFunction($function['category'],
																				$functionName,
	...
	public function listAllFunctionNames() {
		var_dump('listAllFunctionNames');
		return array_keys(PHPExcelFunctions::$list);
