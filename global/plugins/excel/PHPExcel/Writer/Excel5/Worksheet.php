 ...
 */
namespace plugins\excel\PHPExcel\Writer\Excel5;
...
use plugins\excel\PHPExcel\Style\Border as StyleBorder;
use plugins\excel\PHPExcel\Style\Fill as StyleFill;
use plugins\excel\PHPExcel\Style\Alignment as StyleAlignment;
use plugins\excel\PHPExcel\Style\Font as StyleFont;
use plugins\excel\PHPExcel\Style\Conditional as StyleConditional;
use plugins\excel\PHPExcel\Style\Color as StyleColor;
use plugins\excel\PHPExcel\Style\Protection as StyleProtection;
use plugins\excel\PHPExcel\Shared\Excel5 as SharedExcel5;
use plugins\excel\PHPExcel\Cell\DataType;
use plugins\excel\PHPExcel\Cell\DataValidation;
use plugins\excel\PHPExcel\Shared\SharedString;
use plugins\excel\PHPExcel\Worksheet\PageSetup;
use plugins\excel\PHPExcel\Shared\Font as SharedFont;
use plugins\excel\PHPExcel\RichText;
use plugins\excel\PHPExcel\RichText\Run;
use plugins\excel\PHPExcel\Cell;
use plugins\excel\PHPExcel\Worksheet\SheetView;
use plugins\excel\PHPExcel\Writer\Exception as WriterException;
use plugins\excel\PHPExcel\Shared\Escher as SharedEscher;
use plugins\excel\PHPExcel\Exception;
use plugins\excel\PHPExcel\Worksheet as PHPExcelWorksheet;
...
class Worksheet extends BIFFwriter
{
	 ...
	 * @param PHPExcelWorksheet $phpSheet
	 */
...
//		if ($this->_firstColumnIndex > 255) $this->_firstColumnIndex = 255;
	...
	function close()
	{
		...
		if (($defaultWidth = $_phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
			$defaultWidth = SharedFont::getDefaultColumnWidthByFont($_phpSheet->getParent()->getDefaultStyle()->getFont());
		}
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
		...
		if(!empty($arrConditionalStyles)){
			...
			foreach ($arrConditionalStyles as $cellCoordinate => $conditionalStyles) {
				foreach ($conditionalStyles as $conditional) {
					if($conditional->getConditionType() == StyleConditional::CONDITION_EXPRESSION
						|| $conditional->getConditionType() == StyleConditional::CONDITION_CELLIS){
	...
	private function _writeWindow2()
	{
		...
		$fPaged		 = 1;					 // 2
		$fPageBreakPreview = $this->_phpSheet->getSheetView()->getView() === SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
	...
	private function _writeSetup()
	{
		...
		// Page orientation
		$fLandscape = ($this->_phpSheet->getPageSetup()->getOrientation() == PageSetup::ORIENTATION_LANDSCAPE) ?
			0x0 : 0x1;
	...
	private function _writeHeader()
	{
		...
		*/
		...
		$recordData = SharedString::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddHeader());
		$length = strlen($recordData);
	...
	private function _writeFooter()
	{
		...
		*/
		...
		$recordData = SharedString::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddFooter());
		$length = strlen($recordData);
	...
	private function _writeBreaks()
	{
		...
		foreach ($this->_phpSheet->getBreaks() as $cell => $breakType) {
			...
			switch ($breakType) {
				case PHPExcelWorksheet::BREAK_COLUMN:
					// Add to list of vertical breaks
					$vbreaks[] = Cell::columnIndexFromString($coordinates[0]) - 1;
					break;

				case PHPExcelWorksheet::BREAK_ROW:
					// Add to list of horizontal breaks
					$hbreaks[] = $coordinates[1];
					break;

				case PHPExcelWorksheet::BREAK_NONE:
				default:
					// Nothing to do
					break;
			}
	...
	function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
	{
		...
		if ($x1 >= SharedExcel5::sizeCol($this->_phpSheet, Cell::stringFromColumnIndex($col_start))) {
			$x1 = 0;
		}
		if ($y1 >= SharedExcel5::sizeRow($this->_phpSheet, $row_start + 1)) {
			$y1 = 0;
		}
		...
		while ($width >= SharedExcel5::sizeCol($this->_phpSheet, Cell::stringFromColumnIndex($col_end))) {
			$width -= SharedExcel5::sizeCol($this->_phpSheet, Cell::stringFromColumnIndex($col_end));
			++$col_end;
		}

		// Subtract the underlying cell heights to find the end cell of the image
		while ($height >= SharedExcel5::sizeRow($this->_phpSheet, $row_end + 1)) {
			$height -= SharedExcel5::sizeRow($this->_phpSheet, $row_end + 1);
			++$row_end;
		}
		...
		//
		if (SharedExcel5::sizeCol($this->_phpSheet, Cell::stringFromColumnIndex($col_start)) == 0) {
			return;
		}
		if (SharedExcel5::sizeCol($this->_phpSheet, Cell::stringFromColumnIndex($col_end))   == 0) {
			return;
		}
		if (SharedExcel5::sizeRow($this->_phpSheet, $row_start + 1) == 0) {
			return;
		}
		if (SharedExcel5::sizeRow($this->_phpSheet, $row_end + 1)   == 0) {
			return;
		}

		// Convert the pixel values to the percentage value expected by Excel
		$x1 = $x1	 / SharedExcel5::sizeCol($this->_phpSheet, Cell::stringFromColumnIndex($col_start))   * 1024;
		$y1 = $y1	 / SharedExcel5::sizeRow($this->_phpSheet, $row_start + 1)   *  256;
		$x2 = $width  / SharedExcel5::sizeCol($this->_phpSheet, Cell::stringFromColumnIndex($col_end))	 * 1024; // Distance to right side of object
		$y2 = $height / SharedExcel5::sizeRow($this->_phpSheet, $row_end + 1)	 *  256; // Distance to bottom of object
	...
	private function _writeMsoDrawing()
	{
		...
		if (isset($this->_escher)) {
			$writer = new Escher($this->_escher);
			$data = $writer->close();
	...
	private function _writeDataValidity()
	{
		...
		if (!empty($dataValidationCollection)) {
			...
			foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
				...
				switch ($type) {
					case DataValidation::TYPE_NONE:		$type = 0x00;	break;
					case DataValidation::TYPE_WHOLE:		$type = 0x01;	break;
					case DataValidation::TYPE_DECIMAL:	$type = 0x02;	break;
					case DataValidation::TYPE_LIST:		$type = 0x03;	break;
					case DataValidation::TYPE_DATE:		$type = 0x04;	break;
					case DataValidation::TYPE_TIME:		$type = 0x05;	break;
					case DataValidation::TYPE_TEXTLENGTH:	$type = 0x06;	break;
					case DataValidation::TYPE_CUSTOM:		$type = 0x07;	break;
				}
				$options |= $type << 0;
				...
				switch ($errorStyle) {
					case DataValidation::STYLE_STOP:			$errorStyle = 0x00;		break;
					case DataValidation::STYLE_WARNING:		$errorStyle = 0x01;		break;
					case DataValidation::STYLE_INFORMATION:	$errorStyle = 0x02;		break;
				}
				...
				switch ($operator) {
					case DataValidation::OPERATOR_BETWEEN: $operator = 0x00			;	break;
					case DataValidation::OPERATOR_NOTBETWEEN: $operator = 0x01		;	break;
					case DataValidation::OPERATOR_EQUAL: $operator = 0x02				;	break;
					case DataValidation::OPERATOR_NOTEQUAL: $operator = 0x03			;	break;
					case DataValidation::OPERATOR_GREATERTHAN: $operator = 0x04		;	break;
					case DataValidation::OPERATOR_LESSTHAN: $operator = 0x05			;	break;
					case DataValidation::OPERATOR_GREATERTHANOREQUAL: $operator = 0x06;	break;
					case DataValidation::OPERATOR_LESSTHANOREQUAL: $operator = 0x07	;	break;
				}
				$options |= $operator << 20;
	...
	private function _writePageLayoutView(){
		...
		// The options flags that comprise $grbit
		if($this->_phpSheet->getSheetView()->getView() == SheetView::SHEETVIEW_PAGE_LAYOUT){
			$fPageLayoutView   = 1;
		} else {
			$fPageLayoutView   = 0;
		}
	...
	/**
	 * Write CFRule Record
	 * @param StyleConditional $conditional
	 */
	private function _writeCFRule(StyleConditional $conditional){
		...
		if($conditional->getConditionType() == StyleConditional::CONDITION_EXPRESSION){
			$type = 0x02;
			$operatorType = 0x00;
		} else if($conditional->getConditionType() == StyleConditional::CONDITION_CELLIS){
			$type = 0x01;

			switch ($conditional->getOperatorType()){
				case StyleConditional::OPERATOR_NONE:
					$operatorType = 0x00;
					break;
				case StyleConditional::OPERATOR_EQUAL:
					$operatorType = 0x03;
					break;
				case StyleConditional::OPERATOR_GREATERTHAN:
					$operatorType = 0x05;
					break;
				case StyleConditional::OPERATOR_GREATERTHANOREQUAL:
					$operatorType = 0x07;
					break;
				case StyleConditional::OPERATOR_LESSTHAN:
					$operatorType = 0x06;
					break;
				case StyleConditional::OPERATOR_LESSTHANOREQUAL:
					$operatorType = 0x08;
					break;
				case StyleConditional::OPERATOR_NOTEQUAL:
					$operatorType = 0x04;
					break;
				case StyleConditional::OPERATOR_BETWEEN:
					$operatorType = 0x01;
					break;
		...
		if($numConditions == 1){
			...
			$operand2 = null;
		} else if($numConditions == 2 && ($conditional->getOperatorType() == StyleConditional::OPERATOR_BETWEEN)){
			$szValue1 = ($arrConditions[0] <= 65535 ? 3 : 0x0000);
		...
		// Border
		$bBorderLeft = ($conditional->getStyle()->getBorders()->getLeft()->getColor()->getARGB() == StyleColor::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getLeft()->getBorderStyle() == StyleBorder::BORDER_NONE ? 1 : 0);
		$bBorderRight = ($conditional->getStyle()->getBorders()->getRight()->getColor()->getARGB() == StyleColor::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getRight()->getBorderStyle() == StyleBorder::BORDER_NONE ? 1 : 0);
		$bBorderTop = ($conditional->getStyle()->getBorders()->getTop()->getColor()->getARGB() == StyleColor::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getTop()->getBorderStyle() == StyleBorder::BORDER_NONE ? 1 : 0);
		$bBorderBottom = ($conditional->getStyle()->getBorders()->getBottom()->getColor()->getARGB() == StyleColor::COLOR_BLACK
						&& $conditional->getStyle()->getBorders()->getBottom()->getBorderStyle() == StyleBorder::BORDER_NONE ? 1 : 0);
		...
		if($bFormatFont == 1){
			...
			switch ($conditional->getStyle()->getFont()->getUnderline()){
				case StyleFont::UNDERLINE_NONE             : $dataBlockFont .= pack('C', 0x00); $fontUnderline = 0; break;
				case StyleFont::UNDERLINE_DOUBLE           : $dataBlockFont .= pack('C', 0x02); $fontUnderline = 0; break;
				case StyleFont::UNDERLINE_DOUBLEACCOUNTING : $dataBlockFont .= pack('C', 0x22); $fontUnderline = 0; break;
				case StyleFont::UNDERLINE_SINGLE           : $dataBlockFont .= pack('C', 0x01); $fontUnderline = 0; break;
				case StyleFont::UNDERLINE_SINGLEACCOUNTING : $dataBlockFont .= pack('C', 0x21); $fontUnderline = 0; break;
		...
		if($bFormatAlign == 1){
			...
			switch ($conditional->getStyle()->getAlignment()->getHorizontal()){
				case StyleAlignment::HORIZONTAL_GENERAL 			: $blockAlign = 0; break;
				case StyleAlignment::HORIZONTAL_LEFT				: $blockAlign = 1; break;
				case StyleAlignment::HORIZONTAL_RIGHT				: $blockAlign = 3; break;
				case StyleAlignment::HORIZONTAL_CENTER			: $blockAlign = 2; break;
				case StyleAlignment::HORIZONTAL_CENTER_CONTINUOUS	: $blockAlign = 6; break;
				case StyleAlignment::HORIZONTAL_JUSTIFY			: $blockAlign = 5; break;
			}
			...
			switch ($conditional->getStyle()->getAlignment()->getVertical()){
				case StyleAlignment::VERTICAL_BOTTOM 				: $blockAlign = 2 << 4; break;
				case StyleAlignment::VERTICAL_TOP					: $blockAlign = 0 << 4; break;
				case StyleAlignment::VERTICAL_CENTER				: $blockAlign = 1 << 4; break;
				case StyleAlignment::VERTICAL_JUSTIFY				: $blockAlign = 3 << 4; break;
			}
			$blockAlign |= 0 << 7;
		...
		if($bFormatBorder == 1){
			...
			switch ($conditional->getStyle()->getBorders()->getLeft()->getBorderStyle()){
				case StyleBorder::BORDER_NONE              : $blockLineStyle |= 0x00; break;
				case StyleBorder::BORDER_THIN              : $blockLineStyle |= 0x01; break;
				case StyleBorder::BORDER_MEDIUM            : $blockLineStyle |= 0x02; break;
				case StyleBorder::BORDER_DASHED            : $blockLineStyle |= 0x03; break;
				case StyleBorder::BORDER_DOTTED            : $blockLineStyle |= 0x04; break;
				case StyleBorder::BORDER_THICK             : $blockLineStyle |= 0x05; break;
				case StyleBorder::BORDER_DOUBLE            : $blockLineStyle |= 0x06; break;
				case StyleBorder::BORDER_HAIR              : $blockLineStyle |= 0x07; break;
				case StyleBorder::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08; break;
				case StyleBorder::BORDER_DASHDOT           : $blockLineStyle |= 0x09; break;
				case StyleBorder::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A; break;
				case StyleBorder::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B; break;
				case StyleBorder::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C; break;
				case StyleBorder::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D; break;
			}
			switch ($conditional->getStyle()->getBorders()->getRight()->getBorderStyle()){
				case StyleBorder::BORDER_NONE              : $blockLineStyle |= 0x00 << 4; break;
				case StyleBorder::BORDER_THIN              : $blockLineStyle |= 0x01 << 4; break;
				case StyleBorder::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 4; break;
				case StyleBorder::BORDER_DASHED            : $blockLineStyle |= 0x03 << 4; break;
				case StyleBorder::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 4; break;
				case StyleBorder::BORDER_THICK             : $blockLineStyle |= 0x05 << 4; break;
				case StyleBorder::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 4; break;
				case StyleBorder::BORDER_HAIR              : $blockLineStyle |= 0x07 << 4; break;
				case StyleBorder::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 4; break;
				case StyleBorder::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 4; break;
				case StyleBorder::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 4; break;
				case StyleBorder::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 4; break;
				case StyleBorder::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 4; break;
				case StyleBorder::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 4; break;
			}
			switch ($conditional->getStyle()->getBorders()->getTop()->getBorderStyle()){
				case StyleBorder::BORDER_NONE              : $blockLineStyle |= 0x00 << 8; break;
				case StyleBorder::BORDER_THIN              : $blockLineStyle |= 0x01 << 8; break;
				case StyleBorder::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 8; break;
				case StyleBorder::BORDER_DASHED            : $blockLineStyle |= 0x03 << 8; break;
				case StyleBorder::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 8; break;
				case StyleBorder::BORDER_THICK             : $blockLineStyle |= 0x05 << 8; break;
				case StyleBorder::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 8; break;
				case StyleBorder::BORDER_HAIR              : $blockLineStyle |= 0x07 << 8; break;
				case StyleBorder::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 8; break;
				case StyleBorder::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 8; break;
				case StyleBorder::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 8; break;
				case StyleBorder::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 8; break;
				case StyleBorder::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 8; break;
				case StyleBorder::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 8; break;
			}
			switch ($conditional->getStyle()->getBorders()->getBottom()->getBorderStyle()){
				case StyleBorder::BORDER_NONE              : $blockLineStyle |= 0x00 << 12; break;
				case StyleBorder::BORDER_THIN              : $blockLineStyle |= 0x01 << 12; break;
				case StyleBorder::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 12; break;
				case StyleBorder::BORDER_DASHED            : $blockLineStyle |= 0x03 << 12; break;
				case StyleBorder::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 12; break;
				case StyleBorder::BORDER_THICK             : $blockLineStyle |= 0x05 << 12; break;
				case StyleBorder::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 12; break;
				case StyleBorder::BORDER_HAIR              : $blockLineStyle |= 0x07 << 12; break;
				case StyleBorder::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 12; break;
				case StyleBorder::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 12; break;
				case StyleBorder::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 12; break;
				case StyleBorder::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 12; break;
				case StyleBorder::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 12; break;
				case StyleBorder::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 12; break;
			}
			...
			switch ($conditional->getStyle()->getBorders()->getDiagonal()->getBorderStyle()){
				case StyleBorder::BORDER_NONE              : $blockColor |= 0x00 << 21; break;
				case StyleBorder::BORDER_THIN              : $blockColor |= 0x01 << 21; break;
				case StyleBorder::BORDER_MEDIUM            : $blockColor |= 0x02 << 21; break;
				case StyleBorder::BORDER_DASHED            : $blockColor |= 0x03 << 21; break;
				case StyleBorder::BORDER_DOTTED            : $blockColor |= 0x04 << 21; break;
				case StyleBorder::BORDER_THICK             : $blockColor |= 0x05 << 21; break;
				case StyleBorder::BORDER_DOUBLE            : $blockColor |= 0x06 << 21; break;
				case StyleBorder::BORDER_HAIR              : $blockColor |= 0x07 << 21; break;
				case StyleBorder::BORDER_MEDIUMDASHED      : $blockColor |= 0x08 << 21; break;
				case StyleBorder::BORDER_DASHDOT           : $blockColor |= 0x09 << 21; break;
				case StyleBorder::BORDER_MEDIUMDASHDOT     : $blockColor |= 0x0A << 21; break;
				case StyleBorder::BORDER_DASHDOTDOT        : $blockColor |= 0x0B << 21; break;
				case StyleBorder::BORDER_MEDIUMDASHDOTDOT  : $blockColor |= 0x0C << 21; break;
				case StyleBorder::BORDER_SLANTDASHDOT      : $blockColor |= 0x0D << 21; break;
			}
		...
		if($bFormatFill == 1){
			...
			switch ($conditional->getStyle()->getFill()->getFillType()){
				case StyleFill::FILL_NONE						: $blockFillPatternStyle = 0x00; break;
				case StyleFill::FILL_SOLID					: $blockFillPatternStyle = 0x01; break;
				case StyleFill::FILL_PATTERN_MEDIUMGRAY		: $blockFillPatternStyle = 0x02; break;
				case StyleFill::FILL_PATTERN_DARKGRAY			: $blockFillPatternStyle = 0x03; break;
				case StyleFill::FILL_PATTERN_LIGHTGRAY		: $blockFillPatternStyle = 0x04; break;
				case StyleFill::FILL_PATTERN_DARKHORIZONTAL	: $blockFillPatternStyle = 0x05; break;
				case StyleFill::FILL_PATTERN_DARKVERTICAL		: $blockFillPatternStyle = 0x06; break;
				case StyleFill::FILL_PATTERN_DARKDOWN			: $blockFillPatternStyle = 0x07; break;
				case StyleFill::FILL_PATTERN_DARKUP			: $blockFillPatternStyle = 0x08; break;
				case StyleFill::FILL_PATTERN_DARKGRID			: $blockFillPatternStyle = 0x09; break;
				case StyleFill::FILL_PATTERN_DARKTRELLIS		: $blockFillPatternStyle = 0x0A; break;
				case StyleFill::FILL_PATTERN_LIGHTHORIZONTAL	: $blockFillPatternStyle = 0x0B; break;
				case StyleFill::FILL_PATTERN_LIGHTVERTICAL	: $blockFillPatternStyle = 0x0C; break;
				case StyleFill::FILL_PATTERN_LIGHTDOWN		: $blockFillPatternStyle = 0x0D; break;
				case StyleFill::FILL_PATTERN_LIGHTUP			: $blockFillPatternStyle = 0x0E; break;
				case StyleFill::FILL_PATTERN_LIGHTGRID		: $blockFillPatternStyle = 0x0F; break;
				case StyleFill::FILL_PATTERN_LIGHTTRELLIS		: $blockFillPatternStyle = 0x10; break;
				case StyleFill::FILL_PATTERN_GRAY125			: $blockFillPatternStyle = 0x11; break;
				case StyleFill::FILL_PATTERN_GRAY0625			: $blockFillPatternStyle = 0x12; break;
				case StyleFill::FILL_GRADIENT_LINEAR			: $blockFillPatternStyle = 0x00; break;	// does not exist in BIFF8
				case StyleFill::FILL_GRADIENT_PATH			: $blockFillPatternStyle = 0x00; break;	// does not exist in BIFF8
		...
		if($bFormatProt == 1){
			...
			if($conditional->getStyle()->getProtection()->getLocked() == StyleProtection::PROTECTION_PROTECTED){
				$dataBlockProtection = 1;
			}
			if($conditional->getStyle()->getProtection()->getHidden() == StyleProtection::PROTECTION_PROTECTED){
