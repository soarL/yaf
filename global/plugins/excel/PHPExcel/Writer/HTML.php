 ...
 */
namespace plugins\excel\PHPExcel\Writer;
...
use plugins\excel\PHPExcel\Chart;
use plugins\excel\PHPExcel\Shared\SharedString;
use plugins\excel\PHPExcel\Calculation;
 ...
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class HTML extends WriterAbstract implements IWriter {
	/**
	 * PHPExcel object
	 *
	 * @var PHPExcel
	...
	private function _mapVAlign($vAlign) {
		switch ($vAlign) {
			case Alignment::VERTICAL_BOTTOM:		return 'bottom';
			case Alignment::VERTICAL_TOP:		return 'top';
			case Alignment::VERTICAL_CENTER:
			case Alignment::VERTICAL_JUSTIFY:	return 'middle';
	...
	private function _mapHAlign($hAlign) {
		switch ($hAlign) {
			case Alignment::HORIZONTAL_GENERAL:				return false;
			case Alignment::HORIZONTAL_LEFT:					return 'left';
			case Alignment::HORIZONTAL_RIGHT:				return 'right';
			case Alignment::HORIZONTAL_CENTER:
			case Alignment::HORIZONTAL_CENTER_CONTINUOUS:	return 'center';
			case Alignment::HORIZONTAL_JUSTIFY:				return 'justify';
	...
	private function _mapBorderStyle($borderStyle) {
		switch ($borderStyle) {
			case Border::BORDER_NONE:				return 'none';
			case Border::BORDER_DASHDOT:				return '1px dashed';
			case Border::BORDER_DASHDOTDOT:			return '1px dotted';
			...
			case Border::BORDER_MEDIUMDASHDOTDOT:	return '2px dotted';
			case Border::BORDER_MEDIUMDASHED:		return '2px dashed';
			case Border::BORDER_SLANTDASHDOT:		return '2px dashed';
		...
		foreach ($pSheet->getChartCollection() as $chart) {
			if ($chart instanceof Chart) {
				...
				if ($chartCoordinates['cell'] == $coordinates) {
					$chartFileName = SharedFile::sys_get_temp_dir().'/'.uniqid().'.png';
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		foreach ($sheets as $sheet) {
			...
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				if (($width = SharedDrawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
			...
			if ($rowDimension->getRowHeight() == -1) {
				$pt_height = SharedFont::getDefaultRowHeightByFont($this->_phpExcel->getDefaultStyle()->getFont());
			} else {
	 ...
	 * @return	array
	 */
	private function _createCSSStyleAlignment(Alignment $pStyle) {
		// Construct CSS
		$css = array();
	 ...
	 * @return	array
	 */
	private function _createCSSStyleFont(Font $pStyle) {
		// Construct CSS
		$css = array();
		...
		} else if ($pStyle->getUnderline() != Font::UNDERLINE_NONE) {
	 ...
	 * @return	array
	 */
	private function _createCSSStyleBorders(Borders $pStyle) {
		// Construct CSS
		$css = array();
	 ...
	 * @return	string
	 */
	private function _createCSSStyleBorder(Border $pStyle) {
		// Create CSS
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	 ...
	 * @return	array
	 */
	private function _createCSSStyleFill(Fill $pStyle) {
		// Construct HTML
		$css = array();
	...
	private function _setMargins(Worksheet $pSheet) {
		...
		$left = SharedString::FormatNumber($pSheet->getPageMargins()->getLeft()) . 'in; ';
		$htmlPage .= 'left-margin: ' . $left;
		$htmlBody .= 'left-margin: ' . $left;
		$right = SharedString::FormatNumber($pSheet->getPageMargins()->getRight()) . 'in; ';
		$htmlPage .= 'right-margin: ' . $right;
		$htmlBody .= 'right-margin: ' . $right;
		$top = SharedString::FormatNumber($pSheet->getPageMargins()->getTop()) . 'in; ';
		$htmlPage .= 'top-margin: ' . $top;
		$htmlBody .= 'top-margin: ' . $top;
		$bottom = SharedString::FormatNumber($pSheet->getPageMargins()->getBottom()) . 'in; ';
