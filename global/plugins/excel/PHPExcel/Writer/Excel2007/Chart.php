...
use plugins\excel\PHPExcel\Chart as PHPExcelChart;
use plugins\excel\PHPExcel\Chart\DataSeries;
use plugins\excel\PHPExcel\Shared\SharedString;
use plugins\excel\PHPExcel\Chart\Legend;
use plugins\excel\PHPExcel\Chart\PlotArea;
use plugins\excel\PHPExcel\Chart\Title;
use plugins\excel\PHPExcel\Chart\Layout;
...
class Chart extends WriterPart
{
	 ...
	 */
	public function writeChart(PHPExcelChart $pChart = null)
	{
		// Create XML writer
		$objWriter = null;
	 ...
	 * @throws 	WriterException
	 */
	private function _writeTitle(Title $title = null, $objWriter)
	{
		if (is_null($title)) {
			return;
		}
	 ...
	 * @throws 	WriterException
	 */
	private function _writeLegend(Legend $legend = null, $objWriter)
	{
		if (is_null($legend)) {
			return;
		}
	 ...
	 * @throws 	WriterException
	 */
	private function _writePlotArea(PlotArea $plotArea,
									Title $xAxisLabel = NULL,
									Title $yAxisLabel = NULL,
									$objWriter,
									PHPExcelWorksheet $pSheet)
	{
		if (is_null($plotArea)) {
			return;
		}
...
//					} while ($id1 == $id2);
					...
					if (($chartType !== DataSeries::TYPE_PIECHART) &&
						($chartType !== DataSeries::TYPE_PIECHART_3D) &&
						($chartType !== DataSeries::TYPE_DONUTCHART)) {
	 ...
	 * @throws 	WriterException
	 */
	private function _writeCatAx($objWriter, PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
										$objWriter->writeRawData(SharedString::ControlCharacterPHP2OOXML( $caption ));
									$objWriter->endElement();
	 ...
	 * @throws 	WriterException
	 */
	private function _writeValAx($objWriter, PlotArea $plotArea, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
	 ...
	 * @throws 	WriterException
	 */
	private function _writeBubbles($plotSeriesValues, $objWriter, PHPExcelWorksheet $pSheet)
	{
		if (is_null($plotSeriesValues)) {
			return;
		}
	 ...
	 * @throws 	WriterException
	 */
	private function _writeLayout(Layout $layout = NULL, $objWriter)
	{
