...
class Financial {
	...
	private static function _coupFirstPeriodDate($settlement, $maturity, $frequency, $next)
	{
		...
		$result = SharedDate::ExcelToPHPObject($maturity);
		$eom = self::_lastDayOfMonth($result);

		while ($settlement < SharedDate::PHPToExcel($result)) {
		...
		return SharedDate::PHPToExcel($result);
	...
	private static function _daysPerYear($year, $basis=0)
	{
		switch ($basis) {
			...
			case 3 :
				...
				break;
			case 1 :
				$daysPerYear = (DateTime::_isLeapYear($year)) ? 366 : 365;
				break;
			default	:
	...
	public static function AMORLINC($cost, $purchased, $firstPeriod, $salvage, $period, $rate, $basis=0) {
		...
		if (($basis == 1) && ($yearFrac < 1) && (DateTime::_isLeapYear($purchasedYear))) {
			$yearFrac *= 365 / 366;
		}
	...
	public static function COUPDAYBS($settlement, $maturity, $frequency, $basis=0) {
		...
		if (is_string($settlement = DateTime::_getDateValue($settlement))) {
			return Functions::VALUE();
		}
		if (is_string($maturity = DateTime::_getDateValue($maturity))) {
			return Functions::VALUE();
		}
		...
		$daysPerYear = self::_daysPerYear(DateTime::YEAR($settlement),$basis);
	...
	public static function COUPDAYS($settlement, $maturity, $frequency, $basis=0) {
		...
		switch ($basis) {
			...
			case 1: // Actual/actual
					if ($frequency == 1) {
						$daysPerYear = self::_daysPerYear(DateTime::YEAR($maturity),$basis);
	...
	public static function IRR($values, $guess = 0.1) {
		if (!is_array($values)) return Functions::VALUE();
		...
		if (($f1 * $f2) > 0.0) return Functions::VALUE();
	...
	public static function TBILLEQ($settlement, $maturity, $discount) {
		...
		if (Functions::getCompatibilityMode() == Functions::COMPATIBILITY_OPENOFFICE) {
			...
			$daysBetweenSettlementAndMaturity = (DateTime::_getDateValue($maturity) - DateTime::_getDateValue($settlement));
		}
	...
	public static function XIRR($values, $dates, $guess = 0.1) {
		if ((!is_array($values)) && (!is_array($dates))) return Functions::VALUE();
		$values	= Functions::flattenArray($values);
		$dates	= Functions::flattenArray($dates);
		$guess = Functions::flattenSingleValue($guess);
		if (count($values) != count($dates)) return Functions::NaN();
	...
	public static function XNPV($rate, $values, $dates) {
		$rate = Functions::flattenSingleValue($rate);
		if (!is_numeric($rate)) return Functions::VALUE();
		if ((!is_array($values)) || (!is_array($dates))) return Functions::VALUE();
		$values	= Functions::flattenArray($values);
		$dates	= Functions::flattenArray($dates);
		$valCount = count($values);
		if ($valCount != count($dates)) return Functions::NaN();
		if ((min($values) > 0) || (max($values) < 0)) return Functions::VALUE();

		$xnpv = 0.0;
		for ($i = 0; $i < $valCount; ++$i) {
			if (!is_numeric($values[$i])) return Functions::VALUE();
