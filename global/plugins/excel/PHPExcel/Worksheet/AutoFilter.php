 ...
 */
namespace plugins\excel\PHPExcel\Worksheet;
...
use plugins\excel\Calculation;
use plugins\excel\PHPExcel\Shared\Date as SharedDate;
use plugins\excel\PHPExcel\Calculation\DateTime;
...
class AutoFilter
{
	...
	public function getColumn($pColumn) {
		...
		if (!isset($this->_columns[$pColumn])) {
			$this->_columns[$pColumn] = new Column($pColumn, $this);
		}
	...
	public function setColumn($pColumn)
	{
		if ((is_string($pColumn)) && (!empty($pColumn))) {
			$column = $pColumn;
		} elseif(is_object($pColumn) && ($pColumn instanceof Column)) {
	...
	private static function _filterTestInDateGroupSet($cellValue,$dataSet)
	{
		...
		if (is_numeric($cellValue)) {
			$dateValue = SharedDate::ExcelToPHP($cellValue);
			if ($cellValue < 1) {
				//	Just the time part
	...
	private static function _filterTestInCustomDataSet($cellValue, $ruleSet)
	{
		...
		$returnVal = ($join == Column::AUTOFILTER_COLUMN_JOIN_AND);
		foreach($dataSet as $rule) {
			...
			switch ($join) {
				case Column::AUTOFILTER_COLUMN_JOIN_OR :
					...
					if ($returnVal)
						return $returnVal;
					break;
				case Column::AUTOFILTER_COLUMN_JOIN_AND :
					$returnVal = $returnVal && $retVal;
					break;
			}
		}
	...
	private function _dynamicFilterDateRange($dynamicRuleType, &$filterColumn)
	{
		...
		switch ($dynamicRuleType) {
			...
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_TOMORROW :
				$maxVal = (int) SharedDate::PHPtoExcel(strtotime('+1 day',$baseDate));
				$val = (int) SharedDate::PHPToExcel($baseDate);
				break;
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_YEARTODATE :
				$maxVal = (int) SharedDate::PHPtoExcel(strtotime('+1 day',$baseDate));
				$val = (int) SharedDate::PHPToExcel(gmmktime(0,0,0,1,1,date('Y',$baseDate)));
				break;
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_THISYEAR :
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTYEAR :
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTYEAR :
				$maxVal = (int) SharedDate::PHPToExcel(gmmktime(0,0,0,31,12,date('Y',$baseDate)));
				++$maxVal;
				$val = (int) SharedDate::PHPToExcel(gmmktime(0,0,0,1,1,date('Y',$baseDate)));
				break;
			...
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTQUARTER :
				...
				$maxVal = (int) SharedDate::PHPtoExcel(gmmktime(0,0,0,date('t',$baseDate),(1+$thisQuarter)*3,date('Y',$baseDate)));
				++$maxVal;
				$val = (int) SharedDate::PHPToExcel(gmmktime(0,0,0,1,1+$thisQuarter*3,date('Y',$baseDate)));
				break;
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_THISMONTH :
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTMONTH :
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTMONTH :
				$maxVal = (int) SharedDate::PHPtoExcel(gmmktime(0,0,0,date('t',$baseDate),date('m',$baseDate),date('Y',$baseDate)));
				++$maxVal;
				$val = (int) SharedDate::PHPToExcel(gmmktime(0,0,0,1,date('m',$baseDate),date('Y',$baseDate)));
				break;
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_THISWEEK :
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTWEEK :
			case Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTWEEK :
				$dayOfWeek = date('w',$baseDate);
				$val = (int) SharedDate::PHPToExcel($baseDate) - $dayOfWeek;
				$maxVal = $val + 7;
				break;
		}
		...
		$ruleValues[] = array( 'operator' => Rule::AUTOFILTER_COLUMN_RULE_GREATERTHANOREQUAL,
							   'value' => $val
							 );
		$ruleValues[] = array( 'operator' => Rule::AUTOFILTER_COLUMN_RULE_LESSTHAN,
							   'value' => $maxVal
							 );
		...
		return array(
			...
			'arguments' => array( 'filterRules' => $ruleValues,
								  'join' => Column::AUTOFILTER_COLUMN_JOIN_AND
								)
		);
	}
	...
	private function _calculateTopTenValue($columnID,$startRow,$endRow,$ruleType,$ruleValue) {
		...
		$dataValues = Functions::flattenArray(
...
//		echo 'AutoFilter Heading Row ',$rangeStart[1],' is always SHOWN',PHP_EOL;
		...
		foreach($this->_columns as $columnID => $filterColumn) {
			...
			switch ($filterColumn->getFilterType()) {
				case Column::AUTOFILTER_FILTERTYPE_FILTER :
					$ruleValues = array();
					...
					break;
				case Column::AUTOFILTER_FILTERTYPE_CUSTOMFILTER :
					...
					break;
				case Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER :
					$ruleValues = array();
					foreach($rules as $rule) {
					...
					break;
				case Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER :
					$ruleValues = array();
...
//				echo 'Value is ',$cellValue,PHP_EOL;
				...
				$result = $result &&
					call_user_func_array(
						array(PHPEXCEL_SPACE.'PHPExcel\Worksheet\AutoFilter',$columnFilterTest['method']),
						array(
							$cellValue,
