...
class UpdateBankForm extends \Form {
	...
	public function rules() {
		return [
			[['id', 'bankNum', 'bank', 'province', 'city', 'subbranch', 'isDefault'], 'required'],
			['id', 'validateBankId'],
			...
			if(!preg_match("/^\d+$/",$this->bankNum)){
				...
				if($bankAccount->agreeID!=0) {
					...
					if($bankAccount->bankNum!=$this->bankNum) {
						$this->addError('bankNum', '您使用了认证支付，卡号无法修改！');
					}
					if($bankAccount->bank!=$this->bank) {
						$this->addError('bank', '您使用了认证支付，银行无法修改！');
					}
				}
				if($bankAccount->bankNum!=$this->bankNum) {
