...
class AddBankForm extends \Form {
	...
	public function rules() {
		return [
			[['bankUsername', 'bankNum', 'bank', 'province', 'city', 'subbranch', 'paypass', 'isDefault'], 'required'],
	...
	public function validatePaypass() {
		$user = $this->getUser();
		if(!$user->checkPaypass($this->paypass)) {
			$this->addError('paypass', '支付密码错误！');
		}
	}
	...
	public function insert() {
		if($this->check()) {
			...
			$bank->bankUsername = $this->bankUsername;
			$bank->bank = $this->bank;
